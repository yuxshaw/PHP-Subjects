...
//        return $arr;
    ...
    public static function purchase(array $attributes)
    {
        ...
        Log::info("create order log==".__FUNCTION__."==".__LINE__."==property：",$property);
        /* 如果商品有规格则取规格商品信息 配件除外 property */
        $prod = Products::where('goods_id', $product)->first();
        $prod_arr = $prod ? $prod->toArray() : [];
        Log::info("create order log==".__FUNCTION__."==".__LINE__."==prod：",$prod_arr);
        if (Attribute::is_property($property) && !empty($prod)) {
            $product_info = Products::get_products_info($product, $property);
            Log::info("create order log==".__FUNCTION__."==".__LINE__."==is_property is not prod==product_info:",$product_info->toArray());
        }
        if (empty($product_info)) {
            Log::info("create order log==".__FUNCTION__."==".__LINE__."==product_info is empty");
        ...
        Log::info("create order log==".__FUNCTION__."==".__LINE__."==创建order之前，order:",$order);
